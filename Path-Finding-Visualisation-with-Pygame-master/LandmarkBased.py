...
def has_line_of_sight(p1, p2, grid):
    x1, y1 = p1
    x2, y2 = p2
    line_of_sight = abs(x1 - x2) + abs(y1 - y2)
    obstructed = True
    for i in range(1, line_of_sight):
        x = x1 + round(i * (x2 - x1) / line_of_sight)
        y = y1 + round(i * (y2 - y1) / line_of_sight)
        if check_barrier(x, y, grid) is True:
            obstructed = False
            break
...
def reconstruct_path(came_from, ROWS, end, grid):
    gap = WIDTH // ROWS
    for current in came_from:
        pointstart = came_from[current]
        break
    for current in came_from:
        current = came_from[current]
        if has_line_of_sight(pointstart.get_pos(), current.get_pos(), grid):
            continue
        else:
            next = current
            pygame.draw.line(WIN, PURPLE, (pointstart.get_row() * gap, current.get_col() * gap), (next.get_row() * gap, end.get_col() * gap), 5)
            pygame.display.flip()
            pointstart = next
        if current == end:
            next = end
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_SPACE and start and end:
                    ...
                    algorithm(lambda: draw(win, grid, ROWS, width), grid, start, end, ROWS)
                    while True:
                        reconstruct_path(came_from, ROWS, end, grid)
